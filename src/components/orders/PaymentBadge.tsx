...
const config: Record<PaymentVerificationStatus, { label: string; classes: string }> = {
  ...
  pending_review: { label: 'Pendiente de revisión', classes: 'bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-200' },
...
export default function PaymentBadge({ status }: PaymentBadgeProps) {
  if (!status) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-gray-50 px-2.5 py-0.5 text-xs font-medium text-gray-500 ring-1 ring-inset ring-gray-200">
